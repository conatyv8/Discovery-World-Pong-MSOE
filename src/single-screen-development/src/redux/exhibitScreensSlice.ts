...
/**
 * The exhibitScreensSlice stores the following information:
 *  Which tab is being displayed
 *  Which screens to display
 *  The count for each screen's notification badge
 */
export const exhibitScreensSlice = createSlice({
  ...
  reducers: {
    ...
    setContainerLogCount: (
      state,
      action: PayloadAction<{ container: string; value: number }>
    ) => {
      state.newContainerLogCounts[action.payload.container] =
    ...
    incrementContainerLogCount: (state, action: PayloadAction<string>) => {
      state.newContainerLogCounts[action.payload] =
        state.newContainerLogCounts[action.payload] + 1;
    },
  },
});
